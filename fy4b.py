...
if __name__ == "__main__":
    ...
    # print(data.attrs['Begin Line Number'], data.attrs['End Line Number'], data.attrs['Begin Pixel Number'], data.attrs['End Pixel Number'])
    # BLN = data.attrs['Begin Line Number']
    # ELN = data.attrs['End Line Number']
    # BPN = data.attrs['Begin Pixel Number']
    # EPN = data.attrs['End Pixel Number']
    # ds = xr.open_dataset(filename, group='/Data', decode_cf=False)
    # dn_values = ds['NOMChannel01'][:]
    # line = [i for i in range(BLN, ELN + 1)]
    # column = [i for i in range(BPN, EPN + 1)]
    # a = xr.DataArray(dn_values, coords=[line, column], dims=['line', 'column'])
    # print(a)

    # dn_values = dn_values.reset_coords()  # 删除所有坐标
    # print(dn_values)
    # line = xr.IndexVariable('line', range(BLN, ELN + 1))
    # column = xr.IndexVariable('column', range(BPN, EPN + 1))
    # dn_values = dn_values.assign_coords(line=line, column=column)
    # dn_values = dn_values.drop_dims(['phony_dim_4'])
    # #
    # # print(dn_values.dims[0], dn_values.dims[1])
    # print(dn_values)

    # filenameA = 'FY4A_20230401/FY4A-_AGRI--_N_REGC_1047E_L1-_FDI-_MULT_NOM_20230401003000_20230401003417_4000M_V0001.HDF'
    # dataa = xr.open_dataset(filenameA)
    # print(dataa['NOMChannel01'])
    # print(dataa.attrs['Begin Line Number'], dataa.attrs['End Line Number'], dataa.attrs['Begin Pixel Number'], dataa.attrs['End Pixel Number'])
    # BLN = dataa.attrs['Begin Line Number']
    # ELN = dataa.attrs['End Line Number']
    # BPN = dataa.attrs['Begin Pixel Number']
    # EPN = dataa.attrs['End Pixel Number']
    # ds = xr.open_dataset(filenameA)
    # dn_values = ds['NOMChannel01']
    # dn_values = dn_values.rename({dn_values.dims[0]: 'line', dn_values.dims[1]: 'column'})
    # dn_values = dn_values.assign_coords(line=range(BLN, ELN+1), column=range(BPN, EPN+1))
    # print(dn_values.dims[0], dn_values.dims[1])
    # print(dn_values)
